...
class GithubRepositoryTest {
    ...
    private val sampleModulePhtContent = """
        (module "pht/module"
            {[name "pht/example/example-github-module"]}
            {[vers "1.0.0"]})
    """.trimIndent()
        ...
        mockEngine = MockEngine.Companion { request ->
            when (request.url.toString()) {
                        ...
                        [
                            {
                                "name": "pht/example/example-github-module",
                                "version": "1.0.0",
                                "git": "https://github.com/Domaman202/example-module.git",
                                "hash": "${createTestZipHash("pht/example/example-github-module", "1.0.0")}",
                            ...
                            },
                            {
                                ...
                                "version": "1.1.0",
                                "git": "https://github.com/Domaman202/example-module.git",
                                "hash": "${createTestZipHash("pht/example/example-github-module", "1.1.0")}",
                                "file": "https://github.com/releases/v1.1.0/module.zip"
                            },
                            {
                                "name": "another/module",
                                "version": "2.0.0",
                                "git": "https://github.com/another/module.git",
                                "hash": "${createTestZipHash("another/module", "2.0.0")}",
                                "file": "https://github.com/releases/v2.0.0/module.zip"
                            }
                        ]
                    """.trimIndent()
                    respond(
                ...
                "https://github.com/releases/v1.0.0/module.zip" -> {
                    val zipBytes = createTestZip("pht/example/example-github-module", "1.0.0")
                    respond(
                ...
                "https://github.com/releases/v1.1.0/module.zip" -> {
                    val zipBytes = createTestZip("pht/example/example-github-module", "1.1.0")
                    respond(
                        content = ByteReadChannel(zipBytes),
                        status = HttpStatusCode.OK,
                        headers = headersOf(HttpHeaders.ContentType, "application/zip")
                ...
                "https://github.com/releases/v2.0.0/module.zip" -> {
                ...
                "https://github.com/releases/no-module-pht/module.zip" -> {
    ...
    fun `find should return correct modules for name and version constraint`() = runTest {
        ...
        assertEquals("pht/example/example-github-module", module.name)
    ...
    fun `download should extract files correctly`() = runTest {
        ...
        val moduleDir = downloadDir.resolve("pht/example/example-github-module")
        ...
        assertTrue(modulePhtContent.contains("name \"pht/example/example-github-module\""))
    ...
    fun `header should throw exception when module dot pht not found`() = runTest {
                ...
                "https://github.com/Domaman202/Project-Sprout-Module-List-Github/raw/refs/heads/master/verified.json" -> {
                    val json = """
                        [{
                            ...
                            "version": "1.0.0",
                            "git": "https://github.com/no-module/module.git",
                            "hash": "${createTestZipHash("no-module/module", "1.0.0")}",
                            "file": "https://github.com/releases/no-module-pht/module.zip"
                        }]
                    """.trimIndent()
                    respond(
                        content = json,
        ...
        val repoWithoutModulePht = GithubRepository(HttpClient(mockEngineWithoutModulePht))
        val constraint = Constraint.parse("1.0.0")
    ...
    fun `synchronous methods should work correctly`() {
        val constraint = Constraint.parse("1.0.0")
        val modules = repository.find("pht/example/example-github-module", constraint)
    ...
    fun `should handle network errors gracefully`() = runTest {
        ...
        val failingRepo = GithubRepository(HttpClient(failingEngine))

        val constraint = Constraint.parse("1.0.0")
        assertThrows<Exception> {
            failingRepo.findAsync("pht/example/example-github-module", constraint)
        }
    }
    ...
    fun `should handle empty JSON response`() = runTest {
        ...
        val emptyRepo = GithubRepository(HttpClient(emptyJsonEngine))
        val constraint = Constraint.parse("1.0.0")
        val modules = emptyRepo.findAsync("pht/example/example-github-module", constraint)
    ...
    fun `should handle malformed JSON response`() = runTest {
        ...
        val malformedRepo = GithubRepository(HttpClient(malformedJsonEngine))
        val constraint = Constraint.parse("1.0.0")

        assertThrows<Exception> {
            malformedRepo.findAsync("pht/example/example-github-module", constraint)
        }
    }
    ...
    private fun createTestZip(moduleName: String, version: String): ByteArray {
        ...
        ZipOutputStream(baos).use { zos ->
            ...
            val moduleContent = when (moduleName) {
                "pht/example/example-github-module" -> sampleModulePhtContent
